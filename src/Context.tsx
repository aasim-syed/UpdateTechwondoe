...
const isPassword =  RegExp(/^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,}$/i)
...
export declare type ValidationSchema = Record<
  string,
  {
    ...
    error?: string
    required?: boolean
    validate?: 'text' | 'number' | 'email' | 'phone' | 'zip' | 'checkbox' | 'select' | 'password' | 'confirmPassword'
    minLength?: number
    maxLength?: number
    helperText?: string
  }
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, checked?: boolean) => {
    ...
    if (validate) {
      switch (validate) {
        ...
        case 'phone':
          if (value && !isPhone.test(value))error = helperText || 'Please enter a valid phone number  10 digit. i.e: xxx-xxx-xxxx'
          break
        ...
        case 'select':
          ...
          break
        case 'password':
          if(value && !isPassword.test(value))error = helperText || 'Enter appropiate password!'
          break
        case 'confirmPassword':
          if(value && !isPassword )error = helperText || 'Check password'
          break
